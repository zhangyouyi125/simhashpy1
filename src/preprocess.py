...
if __name__=="__main__":
    ...
    with open(doc_path) as ins:
        doc_data = ins.read().encode('utf-8').decode('utf8')
    # Tokenization
    doc_tokens = jt.tokens(doc_data)
    words = jt.weight(doc_data)
    # Write to token file
    with open(doc_path[:doc_path.rfind('.')]+'.token', 'w') as outs:
        outs.write('/'.join([token for token in doc_tokens])) 
    
    # hash
    sim = SimhashBuilder(words)
    finger = sim.vectorize(words)
